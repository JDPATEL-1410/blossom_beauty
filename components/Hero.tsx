...
const stats = [
  { icon: FaAward, val: '10+', label: 'Years Exp.' },
    ...
    <section id="hero" className="relative min-h-[100dvh] overflow-hidden bg-cream pt-[80px] sm:pt-[92px] lg:pt-[110px]">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-5 lg:px-8 min-h-[calc(100dvh-80px)] sm:min-h-[calc(100dvh-92px)] lg:min-h-[calc(100dvh-110px)] flex flex-col lg:grid lg:grid-cols-2 lg:items-center">
        ...
        <div className="order-2 lg:order-1 flex flex-col justify-center py-6 sm:py-8 lg:py-0 text-center lg:text-left">
          ...
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
            <span className="inline-flex items-center gap-2 glass-rose px-3 sm:px-4 py-1.5 rounded-full text-[11px] sm:text-[13px] font-semibold tracking-widest text-rose border border-accent/20 shadow-sm">
          ...
          <motion.h1
            ...
            className="font-serif text-[2.2rem] leading-[1.1] sm:text-[2.8rem] md:text-[3.3rem] lg:text-[3.8rem] font-bold text-dark mt-3 sm:mt-4 mb-1 sm:mb-2"
          >
            Bloom Into
            <br />
          ...
          <motion.p
            ...
            className="text-[15.5px] sm:text-[17.5px] md:text-[19px] text-dark/70 max-w-md mx-auto lg:mx-0 mb-4 sm:mb-5 leading-relaxed"
          >
            Specialist threading, waxing, facials, hair styling, bridal makeup &amp; lash services —{' '}
            <span className="text-dark font-medium">bloomed with care, under our blossom tree.</span>
          </motion.p>
          ...
          >
            <button onClick={() => scrollTo('#booking')}
              className="btn-glow animate-pulse-glow flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-rose to-rose-dark text-white px-5 sm:px-8 py-3 sm:py-3.5 rounded-full font-semibold text-[13px] sm:text-sm tracking-wide shadow-lg shadow-rose/25 min-h-[44px]">
              <FaCalendarAlt className="text-[10px] sm:text-xs" /> Book Appointment
            </button>
            <button onClick={() => scrollTo('#services')}
              className="btn-rose-outline flex-1 sm:flex-none flex items-center justify-center gap-2 glass text-rose border border-accent/25 px-5 sm:px-7 py-3 sm:py-3.5 rounded-full font-semibold text-[13px] sm:text-sm tracking-wide min-h-[44px]">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.55 }}
            ...
            <a href="tel:4045931680" className="flex items-center gap-1.5 text-[12px] sm:text-[14px] text-dark/45 hover:text-rose transition-colors">
              <FaPhoneAlt className="text-rose text-[10px] sm:text-[11px]" /> (404) 593-1680
            </a>
            <span className="flex items-center gap-1.5 text-[12px] sm:text-[14px] text-dark/35">
              <FaMapMarkerAlt className="text-rose text-[10px] sm:text-[11px]" /> Douglasville, GA
            </span>
          </motion.div>
          ...
          >
            {stats.map((st, i) => (
              ...
              >
                <st.icon className="text-rose text-[10px] sm:text-[12px] mb-0.5 group-hover:scale-110 transition-transform" />
                <span className="font-serif font-bold text-dark text-[15px] sm:text-[17px] leading-none">{st.val}</span>
                <span className="text-[9px] sm:text-[10.5px] font-medium text-dark/55 uppercase tracking-wider mt-px text-center leading-tight">{st.label}</span>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.6 }}
        ...
        <motion.button onClick={() => scrollTo('#about')}
          ...
          <span className="text-[8px] text-dark/25 font-medium uppercase tracking-widest">Scroll</span>
