...
export default function Booking() {
  ...
  const inputCls = 'w-full px-3 sm:px-3.5 py-2.5 sm:py-3 rounded-xl border border-accent/20 bg-white/50 focus:border-rose focus:ring-2 focus:ring-rose/10 outline-none transition-all placeholder:text-dark/20 text-dark/70';
  const labelCls = 'block text-[10px] sm:text-[11px] font-semibold text-dark/50 mb-1 sm:mb-1.5 uppercase tracking-wider';
  ...
  return (
    <section id="booking" className="relative py-12 sm:py-20 md:py-28 overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        <img src="/images/booking-bg.jpg" alt="" className="w-full h-full object-cover opacity-[0.12]" />
      ...
      <div ref={ref} className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <span className="inline-flex items-center gap-1.5 bg-gradient-to-r from-blush/50 to-lavender-light/30 text-rose px-2.5 sm:px-3 py-1 rounded-full text-[8px] sm:text-[10px] font-semibold tracking-wider mb-2 sm:mb-3 border border-accent/12">
            <FaClipboardList className="text-[7px] sm:text-[8px]" /> APPOINTMENTS
          </span>
          <h2 className="font-serif text-[1.35rem] sm:text-3xl md:text-4xl lg:text-5xl font-bold text-dark mb-1.5 sm:mb-3">
            Reserve Your <span className="text-gradient">Appointment</span>
          </h2>
          <p className="text-dark/40 max-w-md mx-auto text-[11px] sm:text-sm mb-3 sm:mb-4">
          ...
          <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
            {[
              ...
              <span key={item.t} className="flex items-center gap-1 text-[9px] sm:text-xs text-dark/40">
                <item.icon className="text-rose text-[8px] sm:text-[10px]" /> {item.t}
              </span>
            ))}
          </div>
        </motion.div>
          ...
          <div className="flex gap-2 overflow-x-auto hide-scrollbar px-4 sm:px-0 pb-1 snap-x snap-mandatory">
            {perks.map(p => (
              <div key={p.text} className="snap-center flex-shrink-0 glass rounded-lg px-3 py-2 flex items-center gap-2 min-w-0">
                <p.icon className="text-emerald text-[9px] flex-shrink-0" />
                <span className="text-[10px] text-dark/50 font-medium whitespace-nowrap">{p.text}</span>
              </div>
            ))}
            <a href="tel:4045931680" className="snap-center flex-shrink-0 glass-rose rounded-lg px-3 py-2 flex items-center gap-2">
              <FaPhoneAlt className="text-rose text-[9px] flex-shrink-0" />
              <span className="text-[10px] text-rose font-semibold whitespace-nowrap">(404) 593-1680</span>
            </a>
          </div>
        </motion.div>
        ...
        <div className="grid lg:grid-cols-[1fr,280px] gap-4 sm:gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
                ...
                {/* Submit */}
                <button type="submit"
                  className="btn-glow animate-pulse-glow w-full bg-gradient-to-r from-rose to-rose-dark text-white py-3 sm:py-3.5 rounded-full font-semibold text-[13px] sm:text-sm tracking-wide flex items-center justify-center gap-2 shadow-lg shadow-rose/20 min-h-[44px] mt-1">
                  <FaCalendarAlt className="text-[10px] sm:text-xs" />
                  Reserve My Appointment
                </button>

                {/* Security note */}
                <p className="text-center text-[8px] sm:text-[9px] text-dark/25 mt-1">
                  <FaShieldAlt className="inline text-[7px] mr-1" />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            <div className="glass rounded-2xl p-5 text-center">
              ...
              <p className="text-[9px] text-dark/35 mt-1">Mon-Sat during business hours</p>
            </div>
            ...
            <div className="glass rounded-2xl p-5">
              ...
              <ul className="space-y-2">
                {['No booking fees', 'Flexible rescheduling', 'Personalized service', 'Premium products', 'Clean & hygienic studio'].map(t => (
                  <li key={t} className="flex items-center gap-2 text-[11px] text-dark/45">
                    <FaCheckCircle className="text-emerald text-[9px] flex-shrink-0" /> {t}
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="glass-rose rounded-2xl p-5 text-center">
              ...
              <p className="text-[10px] text-dark/40">Special first-visit pricing!</p>
              <button onClick={() => scrollTo('#offers')}
                className="mt-2 text-rose text-[11px] font-semibold hover:text-rose-dark transition-colors flex items-center gap-1 mx-auto">
                View Offers <FaArrowRight className="text-[7px]" />
              </button>
            </div>
          </motion.div>
        </div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <a href="tel:4045931680"
            className="glass rounded-xl py-3 flex items-center justify-center gap-2 text-rose font-semibold text-[12px] sm:text-sm min-h-[44px] active:scale-[0.97] transition-transform">
            <FaPhoneAlt className="text-[10px]" /> Call to Book
          </a>
          <button onClick={() => scrollTo('#offers')}
            className="glass-rose rounded-xl py-3 flex items-center justify-center gap-2 text-rose font-semibold text-[12px] sm:text-sm min-h-[44px] active:scale-[0.97] transition-transform">
            <FaGift className="text-[10px]" /> View Offers
          </button>
        </motion.div>
      </div>
    </section>
