...
const stats = [
  { val: 5, sfx: '+', label: 'Years Exp.', icon: FaTrophy, color: 'from-gold/80 to-gold-light' },
...
export default function About() {
  ...
  return (
    <section id="about" className="relative py-12 sm:py-20 md:py-28 overflow-hidden">
      ...
      <img src="/images/section-flowers.png" alt="" className="floral-corner top-0 right-0 w-32 sm:w-52 opacity-[0.06] rotate-180" />
        ...
        <div className="grid lg:grid-cols-2 gap-6 sm:gap-10 lg:gap-14 items-center">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7 }} className="relative">
            <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden shadow-xl group">
              <img src="/images/about-bg.jpg" alt="Blossom Beauty Room" className="w-full h-[240px] sm:h-[340px] md:h-[420px] lg:h-[460px] object-cover transition-transform duration-700 group-hover:scale-[1.03]" loading="lazy" />
            ...
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.4, type: 'spring' }}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-serif text-lg sm:text-xl text-rose font-bold leading-none">5+ Years</p>
                  <p className="text-[7px] sm:text-[9px] text-dark/45 font-medium">Beauty Excellence</p>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.6, type: 'spring' }}
              ...
              <div>
                ...
                <p className="text-[7px] text-dark/40">Happy Clients</p>
              </div>
            </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.7, delay: 0.1 }} className="mt-2 lg:mt-0">
            <span className="inline-flex items-center gap-1.5 bg-gradient-to-r from-blush/50 to-lavender-light/25 text-rose px-2.5 sm:px-3 py-0.5 sm:py-1 rounded-full text-[8px] sm:text-[10px] font-semibold tracking-wider mb-2 sm:mb-3 border border-accent/10">
              <FaGem className="text-[6px] sm:text-[8px]" /> ABOUT US
            </span>
            <h2 className="font-serif text-[1.3rem] sm:text-3xl md:text-4xl lg:text-[2.6rem] font-bold text-dark mb-3 sm:mb-4 leading-[1.15]">
              Where Beauty <span className="text-gradient">Blossoms</span> Naturally
            </h2>

            <div className="space-y-2.5 sm:space-y-3 text-dark/50 leading-relaxed text-[12px] sm:text-[13.5px] md:text-[15px]">
              <p>At <strong className="text-dark/70">Blossom Beauty Room</strong>, beauty is more than a service — it's an experience. Located inside Bellemay Salon Studios in Douglasville, we provide personalized beauty treatments designed to enhance your confidence.</p>
              <p>Professional expertise, premium products, and genuine care — every treatment is tailored just for you.</p>
            </div>
              ...
              {features.map((f, i) => (
                <motion.div key={f.text} initial={{ opacity: 0, y: 10 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.25 + i * 0.05 }}
                  ...
                  <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-md sm:rounded-lg bg-gradient-to-br from-blush to-accent/15 flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform duration-300">
                    <f.icon className="text-rose text-[8px] sm:text-[10px]" />
                  </div>
                  <span className="text-[10px] sm:text-[12px] font-medium text-dark/55 leading-tight">{f.text}</span>
                </motion.div>
              ))}
            </div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}} transition={{ delay: 0.6 }}
              ...
              <button onClick={() => go('#booking')}
                ...
                <FaCalendarAlt className="text-[9px] sm:text-[10px]" /> Book a Visit
              </button>
              <button onClick={() => go('#services')}
                ...
                Services <FaArrowRight className="text-[7px] sm:text-[8px]" />
              </button>
            </motion.div>
          </motion.div>
        </div>
          ...
          {stats.map((s, i) => (
            <motion.div key={s.label} initial={{ opacity: 0, y: 15 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.5 + i * 0.07 }}
              ...
              <div className={`w-8 h-8 sm:w-9 sm:h-9 mx-auto mb-1.5 sm:mb-2 rounded-lg sm:rounded-xl bg-gradient-to-br ${s.color} flex items-center justify-center group-hover:scale-110 transition-transform duration-300`}>
                <s.icon className="text-white text-[10px] sm:text-xs" />
              </div>
              <p className="font-serif text-xl sm:text-2xl md:text-3xl font-bold text-rose leading-none">
                <Counter target={s.val} suffix={s.sfx} />
              </p>
              <p className="text-[7px] sm:text-[9px] text-dark/35 mt-0.5 sm:mt-1 font-semibold uppercase tracking-wider">{s.label}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
