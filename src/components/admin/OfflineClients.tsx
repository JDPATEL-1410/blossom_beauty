...
import { FaUser, FaPhone, FaCalendarAlt, FaSpa, FaClock, FaTrash, FaPlus, FaEdit, FaTimes, FaCheck, FaVenusMars, FaSave } from 'react-icons/fa';
...
export default function OfflineClients() {
  const [clients, setClients] = useState<OfflineClient[]>(getClients());
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="glass-strong rounded-2xl p-4 sm:p-5 space-y-3 border border-accent/20">
              ...
              {/* Period tracker — SENSITIVE FEATURE */}
          ...
          filtered.map((c, i) => {
            ...
            return (
              <motion.div key={c.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                {c.notes && (
                  <p className="text-[9px] text-dark/40 italic mt-2 pl-1 border-l-2 border-accent/20">{c.notes}</p>
                )}
              </motion.div>
            );
          })
